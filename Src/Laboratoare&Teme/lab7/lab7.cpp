...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int uniformLightPosition = glGetUniformLocation(shader->program, "light_position");
    ...
    int uniformEyePosition = glGetUniformLocation(shader->program, "eye_position");
    glUniform3fv(uniformEyePosition, 1, glm::value_ptr(eyePosition));

    // TODO(student): Set material property uniforms (shininess, kd, ks, object color)
    int uniformShininess = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(uniformShininess, materialShininess);

    int uniformMaterialKd = glGetUniformLocation(shader->program, "material_kd");  // diffuse light
    glUniform1f(uniformMaterialKd, materialKd);

    int uniformMaterialKs = glGetUniformLocation(shader->program, "material_ks");  // specular light
    glUniform1f(uniformMaterialKs, materialKs);

    int uniformObjectColor = glGetUniformLocation(shader->program, "object_color");
    glUniform3fv(uniformObjectColor, 1, glm::value_ptr(color));
